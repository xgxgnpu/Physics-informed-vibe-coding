"""
E14 - SV-SNN vs FBPINN (domain-decomposition / local basis) on a perforated domain
===================================================================================
Addresses R5.5 (local-basis / domain-decomposition methods, e.g. FBPINN) as a baseline.

Domain: [0,1]^2 minus circular hole (center (0.5,0.5), r=0.2)  -> multiply connected.
PDE: Poisson -Lap u = f, u = sin(mu x) sin(mu y), mu = 6 pi, f = 2 mu^2 u.
BC on outer + hole boundary.

Methods:
  * SV-SNN (accelerated, separable grid + interior mask)            -- global spectral
  * FBPINN (4x4 overlapping subdomains, smooth partition-of-unity   -- LOCAL basis /
            windows, small tanh-MLP per subdomain, AD derivatives)     domain decomposition
  * FourierPINN (Fourier features + MLP)                            -- global features
Metrics: masked best L2, error-vs-distance-to-boundary, params, time. 3 seeds. Self-contained.
"""
...
NSUB = 4  # FBPINN subdomains per axis
...
def dist_to_boundary(X, Y):
    return np.minimum(np.minimum.reduce([X, 1 - X, Y, 1 - Y]), np.sqrt((X - CX) ** 2 + (Y - CY) ** 2) - RAD)
...
def build(seed):
    ...
    th = np.linspace(0, 2 * np.pi, N_HOLE_BC, endpoint=False); xh = CX + RAD * np.cos(th); yh = CY + RAD * np.sin(th)
    ...
    return {"X": X, "Y": Y, "ue": ue, "inside_hole": inside_hole,
...
# ---- SV-SNN accelerated (same as E10) ----
def svsnn_run(data, seed):
    ...
    def loss(p):
        r = resid(p); pde = jnp.sum(r ** 2) / jnp.sum(data["mask"]); bc = jnp.mean((fwd_pts(p, xb, yb) - ub) ** 2); return pde + bc
    @jit
    def step(p, st):
    ...
    def masked_l2(p):
        up = np.array(grid(p, data["x1d"], data["y1d"])); m = ~data["inside_hole"]; return l2(up[m], data["ue"][m]), up
...
# ---- shared pointwise helpers ----
def hvp(f, x, v):
    g = lambda z: jax.jvp(f, (z,), (v,))[1]; return jax.jvp(g, (x,), (v,))[1]
def _train_pointwise(name, init, fwd, data, seed):
    ...
    for ep in range(2, EPOCHS):
        ...
        if ep % EVAL_EVERY == 0 or ep == EPOCHS - 1:
            ...
            if e < best: best = e; bp = jax.tree.map(lambda z: z.copy(), p)
    return {"method": name, "best_l2": best, "params": cp(p), "time_s": time.time() - t0, "u_pred": pred(bp)}
...
def fp_run(data, seed):
    ...
    def fwd(p, xy):
        ...
        return h
    return _train_pointwise("FourierPINN", init, fwd, data, seed)


# ---- FBPINN baseline (domain decomposition, local basis) ----
def make_fbpinn(name, nsub, hid):
    def fbpinn_run(data, seed):
        centers = (np.arange(nsub) + 0.5) / nsub  # subdomain centers along each axis
        half = 1.0 / (2 * nsub); ov = 0.6 * half  # overlap extent
        a = jnp.asarray(centers - half - ov, jnp.float32); b = jnp.asarray(centers + half + ov, jnp.float32)
        cen = jnp.asarray(centers, jnp.float32); sig = float(half + ov); delta = 0.6 * half
        def window1d(coord, i):  # smooth bump on [a_i, b_i]
            return jax.nn.sigmoid((coord - a[i]) / delta) * jax.nn.sigmoid((b[i] - coord) / delta)
        def init(key):
            subs = []
            for _ in range(nsub * nsub):
                dims = [2, hid, hid, 1]; layers = []
                for d in range(len(dims) - 1):
                    key, sub = random.split(key); lim = jnp.sqrt(6.0 / (dims[d] + dims[d + 1]))
                    layers.append({"w": random.uniform(sub, (dims[d], dims[d + 1]), minval=-lim, maxval=lim), "b": jnp.zeros((dims[d + 1],))})
                subs.append(layers)
            return {"subs": subs}
        def mlp(layers, z):
            h = z; n = len(layers)
            for d, L in enumerate(layers):
                h = h @ L["w"] + L["b"]
                if d < n - 1: h = jnp.tanh(h)
            return h
        def fwd(p, xy):
            x = xy[:, 0:1]; y = xy[:, 1:2]; num = jnp.zeros((xy.shape[0], 1)); den = jnp.zeros((xy.shape[0], 1)); s = 0
            for i in range(nsub):
                for j in range(nsub):
                    w = window1d(x, i) * window1d(y, j)  # (N,1)
                    z = jnp.concatenate([(x - cen[i]) / sig, (y - cen[j]) / sig], axis=-1)
                    num = num + w * mlp(p["subs"][s], z); den = den + w; s += 1
            return num / (den + 1e-8)
        return _train_pointwise(name, init, fwd, data, seed)
    return fbpinn_run


def error_vs_distance(up, data):
    X, Y = data["X"], data["Y"]; m = ~data["inside_hole"]
    dist = dist_to_boundary(X, Y); err = np.abs(up - data["ue"]); dvals = dist[m]; evals = err[m]
    bins = np.linspace(0, dvals.max(), 21); idx = np.digitize(dvals, bins); centers = []; means = []
    for bb in range(1, len(bins)):
        sel = idx == bb
        if sel.sum() > 5: centers.append(0.5 * (bins[bb - 1] + bins[bb])); means.append(float(evals[sel].mean()))
    return centers, means


def main():
    out = {}
    runners = [svsnn_run, make_fbpinn("FBPINN", 4, 12), make_fbpinn("FBPINN-rich", 6, 16), fp_run]
    for runner in runners:
        ...
        bb = np.array([r["best_l2"] for r in recs]); tt = np.array([r["time_s"] for r in recs])
        c, mn = error_vs_distance(recs[0]["u_pred"], build(0))
        out[name] = {"params": recs[0]["params"], "best_l2_mean": float(bb.mean()), "best_l2_std": float(bb.std()),
                     "best_l2_min": float(bb.min()), "time_s_mean": float(tt.mean()), "dist_centers": c, "err_means": mn}
        d0 = build(0)
        np.savez(os.path.join(SAVE, f"{name}_field.npz"), u_pred=recs[0]["u_pred"], ue=d0["ue"], X=d0["X"], Y=d0["Y"], inside_hole=d0["inside_hole"])
    with open(os.path.join(SAVE, "E14_results.json"), "w") as f: json.dump(out, f, indent=2)
    print("\nSaved E14 to", SAVE)
