"""
SV-SNN Acceleration – Case 5: 2D Helmholtz with Cylindrical Obstacle
=====================================================================
PDE: -Laplacian(u) - kappa^2 * u = f(x,y) on [0,1]^2 minus cylinder
Cylinder: center (0.5, 0.5), radius 0.15
BC:  u = 0 on exterior boundary (square)
     u = sin(kappa*x) * sin(kappa*y) on cylinder boundary
...
KAPPA = 24.0 * np.pi
W_CHAR = KAPPA
# E11 per-case tuning: SV-SNN samples its Fourier frequencies around a center
# w_char = E11_WCHAR_SCALE * KAPPA. The E5 study shows setting the center slightly
# above the true frequency (1.25-1.5x) markedly lowers error on this Helmholtz
# family; the cylinder (case5) benefits from this without changing parameter count.
E11_WCHAR_SCALE = 1.4
FF_DIM = 64
SEED = 42
EPOCHS = 10000
...
def generate_data(seed=SEED):
    ...
    # Exterior BC: 400 points, 100 per side on [0,1]^2, u=0
    n_per_side = N_BC_EXT // 4
    ...
    x_bc_ext = np.vstack(x_bc_ext_list)
    y_bc_ext = np.vstack(y_bc_ext_list)
    u_bc_ext = np.zeros((x_bc_ext.shape[0], 1))

    # Cylinder BC: 200 points on circle boundary, u=exact
    theta = np.linspace(0, 2 * np.pi, N_BC_CYL, endpoint=False)
    x_bc_cyl = (CX + R_CYL * np.cos(theta)).reshape(-1, 1)
    y_bc_cyl = (CY + R_CYL * np.sin(theta)).reshape(-1, 1)
    u_bc_cyl = np.sin(KAPPA * x_bc_cyl) * np.sin(KAPPA * y_bc_cyl)

    # Combined BC for pointwise methods
    x_bc = np.vstack([x_bc_ext, x_bc_cyl])
    y_bc = np.vstack([y_bc_ext, y_bc_cyl])
    u_bc = np.vstack([u_bc_ext, u_bc_cyl])

    # PDE collocation: LHS with 1.3x oversampling, remove cylinder interior
    n_oversample = int(N_PDE * 1.3)
    pde_pts = lhs(2, samples=n_oversample)
    x_pde_all = pde_pts[:, 0:1]
    y_pde_all = pde_pts[:, 1:2]

    dist_sq = (x_pde_all - CX)**2 + (y_pde_all - CY)**2
    outside = (dist_sq >= R_CYL**2).squeeze()
    x_pde = x_pde_all[outside][:N_PDE]
    y_pde = y_pde_all[outside][:N_PDE]
    ...
    valid_mask = ((X_test - CX)**2 + (Y_test - CY)**2 > R_CYL**2)
    ...
    # Cylinder mask on SPINN grid: 1.0 outside, 0.0 inside
    cyl_mask_grid = ((Xc - CX)**2 + (Yc - CY)**2 <= R_CYL**2)
    pde_mask = (~cyl_mask_grid).astype(np.float32)
    f_grid[cyl_mask_grid] = 0.0
    ...
    data['spinn'] = {
        ...
        'pde_mask': jnp.array(pde_mask, dtype=jnp.float32),
        ...
        'x_cyl': jnp.array(x_bc_cyl, dtype=jnp.float32),
        'y_cyl': jnp.array(y_bc_cyl, dtype=jnp.float32),
        'u_cyl': jnp.array(u_bc_cyl, dtype=jnp.float32),
    }
...
# ============================================================
# Utility: L2 relative error (with optional mask)
# ============================================================
def l2_relative_error(u_pred, u_exact, valid_mask=None):
...
def run_svsnn(data):
    """Self-contained SV-SNN training for Helmholtz with cylinder.
    NUM_MODES=6, NUM_FREQ=64, three-level frequency sampling."""
    print(f"\n{'='*60}")
    ...
    for epoch in range(2, EPOCHS):
        ...
        if epoch % EVAL_EVERY == 0 or epoch == EPOCHS - 1:
            ...
            l2_err = l2_relative_error(u_pred_np, data['u_exact_test'], valid_mask)
...
def run_svsnn_accelerated(data):
    ...
    print(f"  Analytic X_n''/Y_n'', vectorized, grid + cylinder mask")
    print(f"{'='*60}")
    ...
    def init_params(key):
        keys = jax.random.split(key, NUM_MODES * 6 + 1)
        ki = 0
        spatial_x_params, spatial_y_params = [], []
        for n in range(NUM_MODES):
            freqs_x = _sample_frequencies(keys[ki], NUM_FREQ, KAPPA * E11_WCHAR_SCALE); ki += 1
            cos_cx = jax.random.normal(keys[ki], (NUM_FREQ,)) * 0.1; ki += 1
            sin_cx = jax.random.normal(keys[ki], (NUM_FREQ,)) * 0.1; ki += 1
            spatial_x_params.append({'freqs': freqs_x, 'cos_c': cos_cx, 'sin_c': sin_cx, 'bias': jnp.zeros(1)})
            freqs_y = _sample_frequencies(keys[ki], NUM_FREQ, KAPPA * E11_WCHAR_SCALE); ki += 1
            cos_cy = jax.random.normal(keys[ki], (NUM_FREQ,)) * 0.1; ki += 1
            sin_cy = jax.random.normal(keys[ki], (NUM_FREQ,)) * 0.1; ki += 1
            spatial_y_params.append({'freqs': freqs_y, 'cos_c': cos_cy, 'sin_c': sin_cy, 'bias': jnp.zeros(1)})
        mode_coeffs = jax.random.normal(keys[ki], (NUM_MODES,)) * 0.1
        return {'spatial_x': spatial_x_params, 'spatial_y': spatial_y_params, 'mode_coeffs': mode_coeffs}
    ...
    f_grid = KAPPA**2 * jnp.sin(KAPPA * X_mesh) * jnp.sin(KAPPA * Y_mesh)

    cyl_mask_grid = ((X_mesh - CX)**2 + (Y_mesh - CY)**2 <= R_CYL**2)
    pde_mask = (~cyl_mask_grid).astype(jnp.float32)
    f_grid = jnp.where(cyl_mask_grid, 0.0, f_grid)
    ...
    def vectorized_pde_residual(params):
        ...
        u_val = jnp.einsum('nm,jm->nj', cX, Y_all)

        residual = -(u_xx + u_yy) - KAPPA**2 * u_val - f_grid
        return residual * pde_mask

    def loss_fn(params):
        residual = vectorized_pde_residual(params)
        n_valid = jnp.sum(pde_mask)
    ...
    u_pred_final = np.array(vectorized_forward_grid(best_params, data['x_test_1d'], data['y_test_1d']))

    print(f"  Training time: {total_time:.1f}s ({ms_per_epoch:.2f} ms/epoch)")
    print(f"  Best L2 error: {best_l2:.4e}")

    return best_params, history, u_pred_final, n_params, total_time
...
def spinn_forward_pointwise(params, x_pts, y_pts):
    """Evaluate SPINN at specific (x_i, y_i) pairs -> (N, 1)."""
    x_emb = spinn_fourier_embed(x_pts, params['W_x'])
    y_emb = spinn_fourier_embed(y_pts, params['W_y'])
    bx = spinn_branch_forward(params['branch_x'], x_emb)
    by = spinn_branch_forward(params['branch_y'], y_emb)
...
def spinn_loss_fn(params, xc, yc, f_grid, xb_list, yb_list,
                  pde_mask, x_cyl, y_cyl, u_cyl):
    """SPINN loss with cylinder masking and cylinder BC."""
    def u_fn_x(x_in):
    ...
    u_xx = hvp_fwdfwd(u_fn_x, xc, tangents_x)
    u_yy = hvp_fwdfwd(u_fn_y, yc, tangents_y).T

    u_grid = spinn_forward(params, xc, yc)
    laplacian = u_xx + u_yy
    residual = (-laplacian - KAPPA**2 * u_grid - f_grid) * pde_mask
    n_valid = jnp.sum(pde_mask)
    pde_loss = jnp.sum(residual**2) / n_valid

    # Exterior BC loss (square boundary)
    ...
    # Cylinder BC loss (pointwise evaluation)
    u_cyl_pred = spinn_forward_pointwise(params, x_cyl, y_cyl)
    cyl_bc_loss = jnp.mean((u_cyl_pred - u_cyl)**2)
    bc_loss = bc_loss + cyl_bc_loss
...
def train_pointwise_method(name, params, loss_fn, predict_fn, data, epochs=EPOCHS):
    """Generic training loop for pointwise methods (SIREN, FourierPINN, PINN)."""
    print(f"\n{'='*60}")
...
def train_spinn(data, epochs=EPOCHS, params=None):
    """Training loop for SPINN with cylinder masking."""
    print(f"\n{'='*60}")
    ...
    pde_mask = spinn_data['pde_mask']
    x_cyl = spinn_data['x_cyl']
    y_cyl = spinn_data['y_cyl']
    u_cyl = spinn_data['u_cyl']
    valid_mask = data.get('valid_mask', None)

    @jit
    def train_step(params, opt_state):
        (loss, (pde_l, bc_l)), grads = jax.value_and_grad(spinn_loss_fn, has_aux=True)(
            params, xc, yc, f_grid, xb_list, yb_list, pde_mask, x_cyl, y_cyl, u_cyl
        )
        updates, opt_state_new = optimizer.update(grads, opt_state, params)
        params_new = optax.apply_updates(params, updates)
        return params_new, opt_state_new, loss, pde_l, bc_l
...
def main():
    print("=" * 60)
    print("Ablation Study: 2D Helmholtz with Cylinder (kappa = 24*pi)")
    print(f"  kappa = {KAPPA:.4f}")
    print(f"  Cylinder: center=({CX}, {CY}), radius={R_CYL}")
    print(f"  Epochs = {EPOCHS}")
    print(f"  LR = {LR}")
    print(f"  N_PDE = {N_PDE}, N_BC_EXT = {N_BC_EXT}, N_BC_CYL = {N_BC_CYL}, N_TEST = {N_TEST}")
    print("=" * 60)

    print("\nGenerating data...")
    data = generate_data()
    print(f"  PDE collocation points: {data['x_pde'].shape[0]}")
    print(f"  BC points (ext + cyl): {data['x_bc'].shape[0]}")
    print(f"  Test grid: {N_TEST}x{N_TEST}, valid points: {data['valid_mask'].sum()}")
    print("  Done.")
...
CASE_INFO = {"id": "case5", "title": "2D Helmholtz cylinder kappa=24pi",
