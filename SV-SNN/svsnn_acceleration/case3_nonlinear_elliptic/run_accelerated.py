...
def generate_pde_data(key):
    ...
    pts = jnp.array(lhs(2, samples=N_PDE, criterion="maximin"))
    x_pde = pts[:, 0:1]
    y_pde = pts[:, 1:2]
    return x_pde, y_pde
...
def run_siren():
    ...
    siren_W_x = _sample_frequencies(k_wx, FF_DIM, W_CHAR).reshape(1, -1)
    siren_W_y = _sample_frequencies(k_wy, FF_DIM, W_CHAR).reshape(1, -1)
    ff_input_dim_siren = 4 * FF_DIM
    LAYERS = [ff_input_dim_siren, 128, 128, 128, 128, 1]
