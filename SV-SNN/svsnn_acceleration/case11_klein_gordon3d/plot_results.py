"""Generate comparison plots for Case 11: Klein-Gordon 3D (2+1D) high-frequency."""
...
METHOD_ORDER = ['SVSNN_accel', 'SVSNN_orig', 'SPINN', 'SIREN', 'FourierPINN', 'PINN']
DISPLAY_NAMES = {
    'SVSNN_accel': 'SV-SNN\n(accelerated)',
    'SVSNN_orig': 'SV-SNN\n(original)',
    'SPINN': 'SPINN',
...
for name in METHOD_ORDER:
    spath = os.path.join(SAVE_DIR, f"{name}_summary.json")
    hpath = os.path.join(SAVE_DIR, f"{name}_history.npz")
    if os.path.exists(spath) and os.path.exists(hpath):
        ...
        print(f"  [WARN] Missing data for {name}, skipping.")

if not available:
    print("No results found. Run run_accelerated.py first.")
    exit(1)
...
ax.set_title(r'Convergence — Klein-Gordon 3D ($\kappa=10\pi$, $\omega=10\pi$)', fontweight='bold')
...
names = [DISPLAY_NAMES[n] for n in available]
colors = [COLORS[n] for n in available]

best_l2 = [summaries[n]['best_l2_error'] for n in available]
...
ms_per_epoch = [summaries[n]['ms_per_epoch'] for n in available]
...
if 'SVSNN_accel' in summaries and 'SVSNN_orig' in summaries:
    ms_accel = summaries['SVSNN_accel']['ms_per_epoch']
    ms_orig = summaries['SVSNN_orig']['ms_per_epoch']
    labels = ['SV-SNN\n(original)', 'SV-SNN\n(accelerated)']
    vals = [ms_orig, ms_accel]
    cols = [COLORS['SVSNN_orig'], COLORS['SVSNN_accel']]
    ...
    speedup = ms_orig / ms_accel if ms_accel > 0 else 0
...
params_list = [summaries[n]['total_params'] for n in available]
best_l2_list = [summaries[n]['best_l2_error'] for n in available]
...
if 'SVSNN_accel' in histories and 'SVSNN_orig' in histories:
    fig, ax = plt.subplots(figsize=(8, 5))
    for name in ['SVSNN_accel', 'SVSNN_orig']:
        h = histories[name]
