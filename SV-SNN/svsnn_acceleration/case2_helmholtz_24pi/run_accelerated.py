"""
SV-SNN Acceleration Experiment: 2D Helmholtz (kappa = 24*pi)
=============================================================
...
# ============================================================
# METHOD 1b: SV-SNN ACCELERATED — analytic derivs + grid eval
# ============================================================
def run_svsnn_accelerated(data):
    print(f"\n{'='*60}")
    print(f"Training SV-SNN (ACCELERATED)")
    print(f"  Analytic X_n''/Y_n'', vectorized modes, separable grid, zero AD")
    print(f"{'='*60}")
    ...
    def _sample_freqs(key, K, w_char):
        n_low = K // 4
        n_char = K // 2
        n_high = K - n_low - n_char
        k1, k2, k3 = jax.random.split(key, 3)
    ...
    def init_params(key):
        ...
        spatial_y_params = []
        for n in range(NUM_MODES):
            freqs_x = _sample_freqs(keys[ki], NUM_FREQ, KAPPA); ki += 1
            ...
            freqs_y = _sample_freqs(keys[ki], NUM_FREQ, KAPPA); ki += 1
    ...
    n_per_side = NC
    xb_left = jnp.zeros((n_per_side, 1))
    xb_right = jnp.ones((n_per_side, 1))
    yb_bottom = jnp.zeros((n_per_side, 1))
    yb_top = jnp.ones((n_per_side, 1))
    yc_bc = jnp.linspace(0, 1, n_per_side).reshape(-1, 1)
    xc_bc = jnp.linspace(0, 1, n_per_side).reshape(-1, 1)
    ...
    def vectorized_pde_residual(params):
        ...
        return -(u_xx + u_yy) - KAPPA**2 * u_val - f_grid

    def bc_loss_grid(params):
        u_left = vectorized_forward(params, xb_left, yc_bc)
        u_right = vectorized_forward(params, xb_right, yc_bc)
        u_bottom = vectorized_forward(params, xc_bc, yb_bottom)
        u_top = vectorized_forward(params, xc_bc, yb_top)
        return jnp.mean(u_left**2) + jnp.mean(u_right**2) + \
               jnp.mean(u_bottom**2) + jnp.mean(u_top**2)

    def loss_fn(params):
        residual = vectorized_pde_residual(params)
        pde_loss = jnp.mean(residual**2)
        b_loss = bc_loss_grid(params)
        return pde_loss + b_loss, (pde_loss, b_loss)
    ...
    u_pred_final = np.array(vectorized_forward_grid(
        best_params, data['x_test_1d'], data['y_test_1d']))
...
def main():
    print("=" * 60)
    print("SV-SNN Acceleration: 2D Helmholtz (kappa = 24*pi)")
